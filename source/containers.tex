...
\rSec2[container.requirements.range]{Range requirements}

\pnum
Ranges are objects which refer to a sequence of other objects using a
pair of iterators accessed by \tcode{begin()} and \tcode{end()}
functions.  Ranges may or may not contain and own these objects.

\pnum
In Table~\ref{tab:containers.range.requirements}, \tcode{R} denotes a
range class that refers to objects of type \tcode{T}.  \tcode{a}
denotes an lvalue of type \tcode{R}.

\begin{libreqtab2}
{Range requirements}
{tab:containers.range.requirements}
\\ \topline
\lhdr{Expression}       &   \rhdr{Return type}   \\ \capsep
\endfirsthead
\continuedcaption\\
\hline
\lhdr{Expression}       &   \rhdr{Return type}   \\ \capsep
...
\tcode{using std::begin; begin(a)}  &
input iterator type whose value type is \tcode{T}   \\ \rowsep

\tcode{using std::end; end(a)}  &
input iterator type whose value type is \tcode{T}  \\

\end{libreqtab2}

\pnum
In a context where namespace \tcode{std} is an associated namespace,
\tcode{begin(a)}
returns an iterator referring to the first element in the range.
\tcode{end(a)} returns an iterator which is the past-the-end value for the
range.  A type is known as a \textit{range type}
if \tcode{common_type<decltype(begin(a)), decltype(end(a))>::type} is an
InputIterator type~(\ref{input.iterators}).  This type is known as
the \textit{iterator type of the range \tcode{a}}.  The value type of this
iterator is also the value type of the range.  \enternote These requirements are
intended to match the requirements on \tcode{_RangeT} in the range-based for
loop~(\ref{stmt.ranged}). \exitnote

\pnum
\tcode{a} is a valid range if and only if \range{begin(a)}{end(a)} is a valid
range.

\pnum
A range \tcode{a} is \textit{empty} if and only if \tcode{begin(a) == end(a)}.
...
\tcode{r} denotes a valid range~(\ref{container.requirements.range}) whose
value type is implicitly convertible to \tcode{value_type},
...
\tcode{X(r);}       &
                    &
  Equivalent to \tcode{X(begin(r), end(r))} \\ \rowsep
...
\tcode{a = r;}     &
  ...
  \tcode{CopyInsertable} into \tcode{X}
  and \tcode{CopyAssignable}.
  Assigns the range \range{begin(r)}{end(r)} into \tcode{a}. All existing
  elements of \tcode{a} are either assigned to or destroyed.\br
  \returns\ \tcode{*this}.
  \\ \rowsep
...
\tcode{a.insert(p, r);}  &
  \tcode{iterator}            &
  \tcode{a.insert(p, begin(r), end(r))}.  \\ \rowsep
...
\tcode{a.assign(r)}    &
  \tcode{void}          &
  \tcode{a.assign(begin(r), end(r))}. \\ \rowsep
...
\begin{codeblock}
template <class Range>          // such as insert()
...
rt fx3(const_iterator i1, const_iterator i2, Range&& range);
\end{codeblock}

are called with a type \tcode{Range} that does not qualify as a
range~(\ref{container.requirements.range}), or the value type of this range is
not convertible to \tcode{value_type}, then these functions and constructor
shall not participate in overload resolution.  If the constructor
or \tcode{operator=} are called with a type \tcode{Range} such
that \tcode{decay<Range>::type} is the type of the container or a type derived
from the container type, this constructor and function shall also not
participate in overload resolution.  Further, if the range is passed as an
rvalue, it is left in an unspecified state after the call.\footnote{This allows
...
\tcode{X(r);}            &
                          &
  Same as \tcode{X(begin(r), end(r))}.  &
  same as \tcode{X(begin(r), end(r))}.  \\ \rowsep
...
\tcode{a = r}     &
  ...
  \tcode{CopyInsertable} into \tcode{X}
  and \tcode{CopyAssignable}.\br
  \effects Assigns the range \range{begin(r)}{end(r)} into \tcode{a}. All
  existing elements of \tcode{a} are either assigned to or destroyed. &
  $N log N$ in general (where $N$ has the value \tcode{distance(begin(r), end(r)) + a.size()});
  linear if \range{begin(r)}{end(r)} is sorted with \tcode{value_comp()}.
  \\ \rowsep
...
\tcode{a.insert(r)}           &
  \tcode{void}                  &
  Equivalent to \tcode{a.insert(begin(r), end(r))}. &
                                          \\ \rowsep
...
\pnum
For every associative container defined in this Clause, if the constructor
...
\tcode{r} denotes a valid range~(\ref{iterator.requirements.general}) whose
value type is implicitly convertible to \tcode{value_type},
...
&   \tcode{X}
&   Same as \tcode{X(begin(r), end(r))}.
&   Same as \tcode{X(begin(r),} \tcode{end(r))}.
\\ \rowsep
%
...
\tcode{a.insert(r)}
&   \tcode{void}
&   Same as \tcode{a.insert(begin(r), end(r))}.
&   Same as \tcode{a.insert(} \tcode{begin(r),} \tcode{end(r))}.
\\ \rowsep
%
\tcode{a.erase(k)}
&   \tcode{size_type}
...
\pnum
For every unordered associative container defined in this Clause, if the
constructor
...
\pnum
\begin{note} The member variable \tcode{elems} is shown for exposition only,
to emphasize that \tcode{array} is a class aggregate.  The name \tcode{elems}
is not part of \tcode{array}'s interface. \end{note}
...
namespace std {
  ...
  public:
    ...
    template <class Range>
      deque(Range&&, const Allocator& = Allocator());
...
namespace std {
  ...
  public:
    ...
    template <class Range>
      forward_list(Range&&, const Allocator& = Allocator());
    ~forward_list();
...
\tcode{Range} is a range type whose value type is implicitly convertible to
\tcode{value_type}.  If the range is passed as an rvalue, it is left in an
unspecified state after the call.

\pnum
\effects \tcode{insert_after(p, begin(range), end(range))}.

\pnum
\returns
An iterator pointing to the last inserted element or \tcode{position} if \tcode{range} is empty.
\end{itemdescr}
...
namespace std {
  ...
  public:
    ...
    template <class Range>
      list(Range&&, const Allocator& = Allocator());
   ~list();
...
\pnum
\remarks \tcode{shrink_to_fit} is a non-binding request to reduce \tcode{capacity()} to \tcode{size()}. \begin{note} The request is non-binding to allow latitude for implementation-specific optimizations. \end{note}
