...
\pnum
Constructors and member functions taking a \tcode{Range} template argument shall
not participate in overload resolution unless \tcode{Range} is a range
type~(\ref{container.requirements.range}) with a value type implicitly
convertible to \tcode{charT}.  \tcode{basic_string(Range\&\& range, const
Allocator\& a = Allocator())} and \tcode{basic_string\& operator=(Range\&\&
range)} shall additionally not participate if \tcode{decay<Range>::type}
is \tcode{basic_string} or a type derived from \tcode{basic_string}.
...
\begin{itemdecl}
template<class Range
  basic_string(Range&& range, const Allocator& a = Allocator());
\end{itemdecl}

\begin{itemdescr}
\pnum
\requires \tcode{range} is a valid range.

\pnum
\effects Same as \tcode{basic_string(begin(range), end(range), a)}.
\end{itemdescr}
...
\pnum
\effects \tcode{*this = basic_string(std::forward<Range>(range))}.
...
\pnum
\effects Calls \tcode{append(std::forward<Range>(range))}.
...
\pnum
\effects Calls \tcode{append(begin(range), end(range))}.
...
\pnum
\effects Calls \tcode{assign(begin(range), end(range))}.
...
\pnum
\effects \tcode{insert(p, begin(range), end(range))}.

\pnum
\returns An iterator which refers to the copy of the first inserted character, or
\tcode{p} if \tcode{range} is empty.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\requires \range{begin()}{i1}, \range{i1}{i2}, and \tcode{range} are valid ranges.

\pnum
\effects Calls \tcode{replace(i1 - begin(), i2 - i1, begin(range), end(range))}.
