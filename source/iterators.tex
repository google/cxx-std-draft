...
A
\term{range}
is either a pair of iterators that designate the beginning and end of
the computation or an object \tcode{r} of range type~(\ref{range.requirements.implementations}) for
which \tcode{begin(r)} and \tcode{end(r)}, when evaluated in the
context described in \ref{range.requirements.implementations}, return iterators that
designate the beginning and end of the computation.
A range \range{i}{i} or \tcode{r} such that \tcode{begin(r) == end(r)}
is an empty range;
in general, a range \range{i}{j} or \tcode{r}
refers to the elements in the data structure starting with the element
pointed to by
\tcode{i} or \tcode{begin(r)}
and up to but not including the element pointed to by
\tcode{j} or \tcode{end(r)}.
Range \range{i}{j}
is valid if and only if
\tcode{j}
is reachable from
\tcode{i}, and range \tcode{r} is valid if and only if \tcode{end(r)}
is reachable from \tcode{begin(r)}.
