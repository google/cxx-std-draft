...
if \tcode{Range} is not a range type.  Additionally, implementations may exclude
these functions from overload resolution if \tcode{Range's} iterator type is not
an Input Iterator type~(\ref{input.iterators}), but the extent to which they do
so is unspecified.
